...
const App = () => {
  ...
  const getData = async (req,res) => {
    const url = "http://localhost:5000/runnerboard/allrunners" ;
    const options = {
      method : "GET"
    }
  ...
  const onHandleForm = async(e) => {
    e.preventDefault()
    let url = "http://localhost:5000/runnerboard/add-runnerboard"
    let options = {
      method : "POST",
      headers : {
  ...
  const onDelete = async(id) => {
    const url = `http://localhost:5000/runnerboard/deleterunner/${id}`
    const options = {
      method : "DELETE",
      headers : {
        "Content-Type" : "application/json"
